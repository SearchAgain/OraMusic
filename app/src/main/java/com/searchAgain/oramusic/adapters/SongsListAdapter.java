...
package com.searchAgain.oramusic.adapters;
...
import com.searchAgain.oramusic.dialogs.AddPlaylistDialog;
...
import com.searchAgain.oramusic.widgets.BubbleTextGetter;
...
public class SongsListAdapter extends RecyclerView.Adapter<SongsListAdapter.ItemHolder> implements BubbleTextGetter {
    ...
    public void onBindViewHolder(final ItemHolder itemHolder, int i) {
        ...
        ImageLoader.getInstance().displayImage(oramusicUtils.getAlbumArtUri(localItem.albumId).toString(), itemHolder.albumArt, new DisplayImageOptions.Builder().cacheInMemory(true).showImageOnFail(R.drawable.ic_empty_music2).resetViewBeforeLoading(true).build());
        if (MusicPlayer.getCurrentAudioId() == localItem.id) {
            itemHolder.title.setTextColor(Config.accentColor(mContext, ateKey));
            if (MusicPlayer.isPlaying()) {
                itemHolder.visualizer.setColor(Config.accentColor(mContext, ateKey));
                itemHolder.visualizer.setVisibility(View.VISIBLE);
                //MusicPlayer.setQueuePosition(5);
                //if(!itemHolder.visualizer.islinked())
                //itemHolder.visualizer.link(MusicPlayer.getAudioSessionId());
                //TODO:some error in visualizer which crashes app when first in list is selected.
               ...
               Visualizer mVisualizer;
                ...
                Visualizer.OnDataCaptureListener captureListener = new Visualizer.OnDataCaptureListener()
                {
                    ...
                    public void onWaveFormDataCapture(Visualizer visualizer, byte[] bytes,
                                                      int samplingRate)
                    {
                        itemHolder.visualizer.updateVisualizer(bytes);
                    }
                    ...
                    public boolean onMenuItemClick(MenuItem item) {
                        switch (item.getItemId()) {
                            case R.id.popup_song_remove_playlist:
                                oramusicUtils.removeFromPlaylist(mContext, arraylist.get(position).id, playlistId);
                                removeSongAt(position);
                                notifyItemRemoved(position);
                                break;
                            case R.id.popup_song_play:
                                MusicPlayer.playAll(mContext, songIDs, position, -1, oramusicUtils.IdType.NA, false);
                                break;
                            case R.id.popup_song_play_next:
                                long[] ids = new long[1];
                                ids[0] = arraylist.get(position).id;
                                MusicPlayer.playNext(mContext, ids, -1, oramusicUtils.IdType.NA);
                                break;
                            ...
                            case R.id.popup_song_addto_queue:
                                ...
                                MusicPlayer.addToQueue(mContext, id, -1, oramusicUtils.IdType.NA);
                                break;
                            case R.id.popup_song_addto_playlist:
                                AddPlaylistDialog.newInstance(arraylist.get(position)).show(mContext.getSupportFragmentManager(), "ADD_PLAYLIST");
                                break;
                            case R.id.popup_song_share:
                               oramusicUtils.shareTrack(mContext, arraylist.get(position).id);
                                break;
                            case R.id.popup_song_delete:
                                long[] deleteIds = {arraylist.get(position).id};
                                oramusicUtils.showDeleteDialog(mContext,arraylist.get(position).title, deleteIds, SongsListAdapter.this, position);
                                break;
                        }
                        return false;
                    }
    ...
    public class ItemHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            handler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    MusicPlayer.playAll(mContext, songIDs, getAdapterPosition(), -1, oramusicUtils.IdType.NA, false);

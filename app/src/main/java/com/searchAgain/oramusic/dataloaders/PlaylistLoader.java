...
public class PlaylistLoader {
    ...
    public static List<Playlist> getPlaylists(Context context, boolean defaultIncluded) {
        ...
        if (mCursor != null && mCursor.moveToFirst()) {
            do {
                ...
                final int songCount = oramusicUtils.getSongCountForPlaylist(context, id);
    ...
    private static void makeDefaultPlaylists(Context context) {
        ...
        /* Last added list */
        final Playlist lastAdded = new Playlist(oramusicUtils.PlaylistType.LastAdded.mId,
                resources.getString(oramusicUtils.PlaylistType.LastAdded.mTitleId), -1);
        mPlaylistList.add(lastAdded);

        /* Recently Played */
        final Playlist recentlyPlayed = new Playlist(oramusicUtils.PlaylistType.RecentlyPlayed.mId,
                resources.getString(oramusicUtils.PlaylistType.RecentlyPlayed.mTitleId), -1);
        mPlaylistList.add(recentlyPlayed);

        /* Top Tracks */
        final Playlist topTracks = new Playlist(oramusicUtils.PlaylistType.TopTracks.mId,
                resources.getString(oramusicUtils.PlaylistType.TopTracks.mTitleId), -1);

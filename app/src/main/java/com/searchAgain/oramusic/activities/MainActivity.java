...
import com.searchAgain.oramusic.fragments.FoldersFragment;
import com.searchAgain.oramusic.fragments.MainFragment;
import com.searchAgain.oramusic.fragments.PlaylistFragment;
import com.searchAgain.oramusic.fragments.QueueFragment;
import com.searchAgain.oramusic.permissions.Nammu;
import com.searchAgain.oramusic.permissions.PermissionCallback;
import com.searchAgain.oramusic.slidinguppanel.SlidingUpPanelLayout;
...
public class MainActivity extends BaseActivity implements ATEActivityThemeCustomizer {
    ...
    final PermissionCallback permissionReadstorageCallback = new PermissionCallback() {
        ...
        public void permissionGranted() {
            checkRecordPermissionAndThenLoad();
        }

        @Override
        public void permissionRefused() {
            if (Nammu.shouldShowRequestPermissionRationale(MainActivity.this, Manifest.permission.READ_EXTERNAL_STORAGE)) {
                Snackbar.make(panelLayout, "oramusic will need to read external storage to display songs on your device.",
    ...
    final PermissionCallback permissionRecordAudioCallback = new PermissionCallback() {
        @Override
        public void permissionGranted() {
                loadEverything();
        }
        ...
        @Override
        public void permissionRefused() {
            if (Nammu.shouldShowRequestPermissionRationale(MainActivity.this, Manifest.permission.RECORD_AUDIO)) {
                Snackbar.make(panelLayout, "oramusic will need to have record permission to listen to your need.",
                        Snackbar.LENGTH_INDEFINITE)
                        .setAction("OK", new View.OnClickListener() {
                            @Override
                            public void onClick(View view) {
                                Nammu.askForPermission(MainActivity.this, Manifest.permission.RECORD_AUDIO, permissionRecordAudioCallback);
                            }
                        }).show();
            } else {
                finish();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (oramusicUtils.isMarshmallow()) {
            checkStoragePermissionAndThenLoad();
        } else {
            loadEverything();
        }
    ...
    //// TODO: 9/30/2017 filter file path in setting
    private void checkStoragePermissionAndThenLoad() {
    ...
    private void checkRecordPermissionAndThenLoad() {
        //check for permission
        if (Nammu.checkPermission(Manifest.permission.RECORD_AUDIO)) {
            ...
            if (Nammu.shouldShowRequestPermissionRationale(this, Manifest.permission.RECORD_AUDIO)) {
                ...
                Nammu.askForPermission(this, Manifest.permission.RECORD_AUDIO, permissionRecordAudioCallback);
            }
        }
    }
    ...
    public void setDetailsToHeader() {
        ...
        ImageLoader.getInstance().displayImage(oramusicUtils.getAlbumArtUri(MusicPlayer.getCurrentAlbumId()).toString(), albumart,

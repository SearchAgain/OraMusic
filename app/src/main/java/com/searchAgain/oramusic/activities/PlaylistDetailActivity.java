...
import com.searchAgain.oramusic.adapters.SongsListAdapter;
import com.searchAgain.oramusic.dataloaders.LastAddedLoader;
...
import com.searchAgain.oramusic.dataloaders.TopTracksLoader;
...
import com.searchAgain.oramusic.widgets.DragSortRecycler;
...
public class PlaylistDetailActivity extends BaseActivity implements ATEActivityThemeCustomizer, ATEToolbarCustomizer {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (animate && oramusicUtils.isLollipop() && PreferencesUtility.getInstance(this).getAnimations()) {
    ...
    private void setAlbumart() {
        ...
        loadBitmap(oramusicUtils.getAlbumArtUri(getIntent().getExtras().getLong(Constants.ALBUM_ID)).toString());
    }
    ...
    private void setRecyclerViewAapter() {
        ...
        if (animate && oramusicUtils.isLollipop() && PreferencesUtility.getInstance(mContext).getAnimations()) {
    ...
    private void clearAutoPlaylists() {
        switch (action) {
            case Constants.NAVIGATE_PLAYLIST_LASTADDED:
                oramusicUtils.clearLastAdded(this);
                break;
            case Constants.NAVIGATE_PLAYLIST_RECENT:
                oramusicUtils.clearRecent(this);
                break;
            case Constants.NAVIGATE_PLAYLIST_TOPTRACKS:
                oramusicUtils.clearTopTracks(this);
                break;
        }
        Intent returnIntent = new Intent();
        setResult(Activity.RESULT_OK, returnIntent);
